...
export async function scheduleApprovedPost(input: { text: string; scheduledFor: string; imagePath?: string; altText?: string }): Promise<ScheduledPost> {
  ...
  const posts = await readQueue(); posts.push(post); await writeQueue(posts); return post;
}

export async function listScheduledPosts(): Promise<ScheduledPost[]> { return (await readQueue()).sort((a,b) => a.scheduledFor.localeCompare(b.scheduledFor)); }

export async function cancelScheduledPost(id: string): Promise<ScheduledPost> {
  const posts = await readQueue(); const post = posts.find((p) => p.id === id);
  ...
  post.status = "cancelled"; await writeQueue(posts); return post;
}
...
export async function claimDuePosts(now = new Date()): Promise<ScheduledPost[]> {
  ...
  for (const post of due) post.status = "publishing";
  if (due.length) await writeQueue(posts);
  return due;
}

export async function completeScheduledPost(id: string, result: { postId?: string; error?: string }): Promise<void> {
  const posts = await readQueue(); const post = posts.find((p) => p.id === id); if (!post) return;
  if (result.error) { post.status = "failed"; post.error = result.error; }
  else { post.status = "published"; post.postId = result.postId; post.publishedAt = new Date().toISOString(); }
