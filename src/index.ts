...
server.tool("render_handwritten_diagram", "Render a reviewed diagram specification to a LinkedIn-compatible local PNG for preview. This never publishes.", diagramSchema, async ({ title, nodes, edges, footer }) => { const diagram = normalizeDiagram({ title, nodes, edges, footer }); const imagePath = await renderHandwrittenDiagram(diagram); return { content: [{ type: "text", text: JSON.stringify({ status: "awaiting_approval", imagePath, mediaType: "image/png", diagram, published: false, next: "Preview this PNG. After approval, publish it with publish_linkedin_post." }) }] }; });
