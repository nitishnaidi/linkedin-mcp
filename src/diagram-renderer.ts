...
export async function renderHandwrittenDiagram(spec: HandwrittenDiagramSpec, outputDir = process.env.LINKEDIN_MCP_OUTPUT_DIR ?? ".linkedin-mcp/diagrams"): Promise<string> {
  ...
  spec.nodes.forEach((node, i) => {
    ...
    const x = columns === 1 ? (WIDTH - NODE_W) / 2 : 170 + col * 560;
    const y = 180 + row * 180;
  ...
  const edgeSvg = spec.edges.map((edge, i) => {
    ...
    return `<path d="M ${x1} ${y1} Q ${(x1+x2)/2 + bend} ${(y1+y2)/2} ${x2} ${y2}" class="edge" marker-end="url(#arrow)"/>${label}`;
  }).join("\n");
  const nodeSvg = spec.nodes.map((node, i) => {
    const p = positions.get(node.id)!;
    const rotate = i % 3 === 0 ? -0.7 : i % 3 === 1 ? 0.5 : -0.2;
...
<defs><marker id="arrow" markerWidth="12" markerHeight="12" refX="10" refY="5" orient="auto"><path d="M0,0 L10,5 L0,10" fill="none" stroke="#222" stroke-width="2"/></marker></defs>
<style>svg{background:#fffdf7}.title,.node-text,.edge-label,.footer{font-family:"Comic Sans MS","Segoe Print","Bradley Hand",cursive;fill:#222}.title{font-size:46px;font-weight:700}.node-text{font-size:25px}.edge-label{font-size:19px}.footer{font-size:22px}.node{fill:#fff;stroke:#222;stroke-width:4;stroke-linecap:round;stroke-dasharray:1 0}.edge{fill:none;stroke:#222;stroke-width:4;stroke-linecap:round}</style>
<text x="600" y="90" text-anchor="middle" class="title">${esc(spec.title)}</text>
${edgeSvg}
${nodeSvg}
${footer}
</svg>`;
  ...
  const path = resolve(dir, `diagram-${randomUUID()}.svg`);
